...
def construct_task_name(explanation):
    ...
    if 'data_--_train_count' in explanation:
        name += extract_query('data_--_train_count', addition='tc')  # seed
    ...
    if 'model_--_depth' in explanation:
        name += extract_query('model_--_depth', addition='dp')  # depth
...
if __name__ == '__main__':
    ...
    parser.set_defaults(pbs=True, force_random=False)
    ...
    for config, explanation in new_configs:
        ...
        for i in range(args.repetition_num):
            ...
            script = script.format(
                ...
                ncpus=(6 if args.pbs else 12) if is_random else (24 if args.pbs else 48),
                # ncpus=(24 if args.pbs else 48),
                ngpus=1 if is_random else 4,
                # ngpus=4,
                gpu_mem='32gb',
                mem=('40gb' if args.pbs else '122G') if is_random else ('187gb' if args.pbs else '490G'),
